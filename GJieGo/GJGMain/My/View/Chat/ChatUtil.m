...
@interface ChatUtil()<EMCallManagerDelegate>
{
    NSTimer *_callTimer;
}
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_LOGINCHANGE object:@NO];
    ...
    //custom
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:NSLocalizedString(@"loginAtOtherDevice1", @"your login account has been logged in other places,please log in again") delegate:self cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
    alertView.tag = 3002;
    [alertView show];
}
...
{
    //[[NSNotificationCenter defaultCenter] postNotificationName:HasUnreadChatMessagesNotification object:aMessages];
    BOOL isRefreshCons = YES;
    for(EMMessage *message in aMessages){
        //custom 注意iOS，Android，pc的key要统一
        //同步把环信的昵称和头像也设置了
        NSLog(@"message: %@, %@, %@, %@",message.conversationId,message.from,message.to,message.ext);
        NSMutableDictionary * hxDict = [NSMutableDictionary dictionary];
        [hxDict setValue:message.ext[@"name"] forKey:@"nickName"];
        [hxDict setValue:message.ext[@"head"]forKey:@"userImage"];
        [hxDict setValue:message.ext[@"userId"] forKey:@"userId"];
        if (message.ext[@"userName"] && [message.ext[@"userName"]length]) {
            [hxDict setValue:message.ext[@"userName"]forKey:@"userName"];
        }else{
            [hxDict setValue:message.from forKey:@"userName"];
        }
        ...
        if (!isAppActivity) {
            ...
            NSString * name;
            NSArray * strArr = [message.ext[@"name"] componentsSeparatedByString:@"-"];
            if (strArr.count == 2) {
                name = strArr.firstObject;
            }else{
            ...
            switch (msgBody.type) {
                case EMMessageBodyTypeText:
                {
                    ...
                    notification.alertBody = [NSString stringWithFormat:@"%@:%@",name,textBody.text];
                }
                    break;
                case EMMessageBodyTypeImage:
                {
                    notification.alertBody = [NSString stringWithFormat:@"%@:%@",name,@"发来一张图片"];
                }
                    break;
            ...
            notification.userInfo = @{@"note":@{@"NoteType" : @100,@"chatId" : message.from}};
            notification.alertAction = NSLocalizedString(@"open", @"Open");
            notification.timeZone = [NSTimeZone defaultTimeZone];
            [[UIApplication sharedApplication] scheduleLocalNotification:notification];
        }else{
            [[NotificationManager shareManager] handleChatMessages:@{@"note":@{@"NoteType" : @100,@"chatId" : message.from}}];
        }
