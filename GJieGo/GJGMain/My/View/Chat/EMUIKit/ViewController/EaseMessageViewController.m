...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (timeCell == nil) {
            ...
            //custom
            timeCell.titleLabelFont = JXFontForNormal(12);
            timeCell.titleLabelColor = JXColorFromRGB(0xbbbbbb);
        }
        ...
        if (sendCell == nil) {
            ...
            sendCell.messageTextFont = JXFontForNormal(12);
            if (model.isSender) {
                sendCell.messageTextColor = JXFfffffColor;
            }else{
                sendCell.messageTextColor = JX999999Color;
            }
...
#pragma mark - 四类个性化设置，这些参数都可以不传，此时会走默认设置
    ...
    imagePickerVc.navigationBar.barTintColor = JXMainColor;
    imagePickerVc.navigationBar.tintColor = JX333333Color;
    imagePickerVc.oKButtonTitleColorDisabled = [UIColor lightGrayColor];
    imagePickerVc.oKButtonTitleColorNormal = JXMainColor;
    imagePickerVc.barItemTextColor = JXTextColor;

    imagePickerVc.navigationBar.backIndicatorImage = [JXImageNamed(@"back") imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    imagePickerVc.navigationBar.backIndicatorTransitionMaskImage = [JXImageNamed(@"back") imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            ...
            //custom
            model.avatarImage = [UIImage imageNamed:@"portrait_default"];
